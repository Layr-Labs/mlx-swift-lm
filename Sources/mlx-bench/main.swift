...
struct MLXBench {
    static func main() async {
        do {
            ...
            case .gatherQMV:
                try runGatherQMVBenchmark(args: args, mode: mode)
            }
        } catch {
            eprint("error: \(error)")
            exit(1)
    ...
    private static func runGatherQMVBenchmark(args: BenchArguments, mode: BenchMode) throws {
    ...
    private static func printDFlashLayerDrift(
        ...
        try printDFlashLayerComponentDrift(
            target: target,
            prompt: prompt,
            baselineTokenIds: baselineTokenIds,
            generatedOffset: generatedOffset,
    ...
    private static func printDFlashLayerComponentDrift(
        target: any DFlashTargetModel,
        prompt: MLXArray,
        baselineTokenIds: [Int],
        generatedOffset: Int,
        ...
        if let direct = target as? Gemma4TextModel {
            textModel = direct
        } else if let wrapper = target as? Gemma4Model {
            textModel = wrapper.textModel
        } else {
            textModel = nil
        }
        guard let textModel else { return }

        let layerIds = [0]
        let sequentialRows = try targetSequentialLayerComponentRowsForGemma4(
            textModel: textModel,
            prompt: prompt,
            baselineTokenIds: baselineTokenIds,
            generatedOffset: generatedOffset,
        ...
        let blockForward = try textModel.dFlashLayerComponents(
            verifyInput,
            cache: verifyCache,
            layerIds: layerIds
        )
        let blockRows = dFlashLayerComponentRows(
            from: blockForward,
            offset: verifyOffset
        )
        eval(
            sequentialRows.attentionRows + sequentialRows.layerRows
                + blockRows.attentionRows + blockRows.layerRows
        )

        let attention = dFlashDriftMetric(
            sequentialRows.attentionRows[0],
            blockRows.attentionRows[0]
        )
        let layer = dFlashDriftMetric(
            sequentialRows.layerRows[0],
            blockRows.layerRows[0]
        )
        print(
            "   component_drift layer=0"
                + " attention_max=\(String(format: "%.5f", attention.maxAbs))"
                + " attention_mean=\(String(format: "%.5f", attention.meanAbs))"
                + " layer_max=\(String(format: "%.5f", layer.maxAbs))"
                + " layer_mean=\(String(format: "%.5f", layer.meanAbs))"
        )
    }

    private static func targetSequentialLayerComponentRowsForGemma4(
        textModel: Gemma4TextModel,
        prompt: MLXArray,
        baselineTokenIds: [Int],
        generatedOffset: Int,
        layerIds: [Int]
    ) throws -> Gemma4LayerComponentRows {
        var generationParameters = GenerateParameters(temperature: 0)
        generationParameters.maxTokens = Swift.max(generatedOffset + 1, 1)
        generationParameters.temperature = 0

        let cache = textModel.newCache(parameters: generationParameters)
        if generatedOffset == 0 {
            let forward = try textModel.dFlashLayerComponents(
                prompt,
                cache: cache,
                layerIds: layerIds
            )
            let offset = prompt.ndim >= 2 ? prompt.dim(1) - 1 : prompt.dim(0) - 1
            return dFlashLayerComponentRows(from: forward, offset: offset)
        }

        _ = textModel.callAsFunction(prompt, cache: cache)
        for token in baselineTokenIds.prefix(Swift.max(0, generatedOffset - 1)) {
            _ = textModel.callAsFunction(tokenRow([token]), cache: cache)
        }
        ...
        return dFlashLayerComponentRows(from: forward, offset: 0)
    }

    private static func dFlashLayerComponentRows(
        from forward: Gemma4DFlashLayerComponentForward,
        offset: Int
    ) -> Gemma4LayerComponentRows {
        Gemma4LayerComponentRows(
            attentionRows: forward.attentionStates.map { $0[0, offset, 0...] },
            layerRows: forward.layerOutputs.map { $0[0, offset, 0...] }
        )
    }
...
private struct Gemma4LayerComponentRows {
    let attentionRows: [MLXArray]
    let layerRows: [MLXArray]
}
...
private func benchmarkPromptTokens(
    text prompt: String,
    args: BenchArguments,
    tokenizer: any MLXLMCommon.Tokenizer
) throws -> [Int32] {
...
private func envBoolOverride(names: [String]) -> Bool? {
...
private func printDecodedOutput(label: String, tokenIds: [Int], tokenizer: MLXLMCommon.Tokenizer) {
...
private func firstTokenMismatch(_ baseline: [Int], _ candidate: [Int]) -> String? {
