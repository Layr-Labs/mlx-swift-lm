...
enum CBv2AttentionV1 {
    ...
    ///
    /// Every SDPA terminal in this module funnels through here, INCLUDING
    /// the compiled decode graph (`CBv2CompiledLayerCache`), which is the
    /// default route on supported hardware and the one the production
    /// adapter opts GPT-OSS — the only sinks model — into.
    ///
