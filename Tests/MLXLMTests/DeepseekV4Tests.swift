...
public class DeepseekV4Tests: XCTestCase {
    ...
    func testModelOutputShape() throws {
        ...
        // num_hidden_layers counts all real layers; MTP (absent here) would be additional.
        XCTAssertEqual(model.model.layers.count, config.numHiddenLayers,
                       "Layer count should equal num_hidden_layers (MTP layers are additional, not subtracted)")
    ...
    func testKVHeadsCount() {
        let config = makeSmallConfig()
        let model = DeepseekV4Model(config)
        XCTAssertEqual(model.kvHeads.count, config.numHiddenLayers)
    ...
    func testSanitizeDropsMTPLayers() {
        ...
        // num_hidden_layers counts ONLY real layers (0..<numHiddenLayers); MTP layers, when
        // present, are ADDITIONAL at indices >= numHiddenLayers. Provide all real layers PLUS
        // a trailing MTP layer; assert real ones survive and the MTP one is dropped.
        var fakeWeights: [String: MLXArray] = [:]
        let mtpIdx = config.numHiddenLayers + config.numNextnPredictLayers - 1  // = 4
        for l in 0 ... mtpIdx {
        ...
        // All real layers kept; trailing MTP layer(s) filtered out.
        for l in 0 ..< config.numHiddenLayers {
            XCTAssertNotNil(sanitized["model.layers.\(l).attn_norm.weight"],
                            "Real layer \(l) should be present")
        }
        for l in config.numHiddenLayers ... mtpIdx {
    ...
    func testSanitizeDropsCompressorIndexerKeys() {
        // Compressor weights feed DeepseekV4Compressor (DSA) and are KEPT when
        // DSA is enabled (default). Indexer (top-k slot pruning) remains
        // unimplemented — attending all slots is exact — so its keys drop.
        let config = makeSmallConfig()
        let model = DeepseekV4Model(config)

        let fakeWeights: [String: MLXArray] = [
        ...
        if dsaEnabled {
            XCTAssertNotNil(sanitized["model.layers.0.attn.compressor.wkv.weight"],
                            "Compressor keys should be KEPT for the DSA path")
            XCTAssertNotNil(sanitized["model.layers.0.attn.compressor.wgate.weight"],
                            "Compressor keys should be KEPT for the DSA path")
        }
...
// MARK: - DSA Compressor golden-vector tests
// Vectors generated by scripts/dsa_golden.py from DeepSeek's OFFICIAL
// inference/model.py Compressor (torch, fp32). The critical invariant
// (prefill-then-decode == one-shot prefill) was asserted there at <1e-5.
public class DSACompressorGoldenTests: XCTestCase {
    static let goldenPath =
        "/Users/gaj/Documents/Research/SSDStream/notes/dsa_golden.json"

    private func goldenConfig() -> DeepseekV4Configuration {
        // dim=16, head_dim=8, rope_head_dim=4, theta=160000, NO YaRN:
        // factor=1 makes dsv4YarnWavelengths collapse to plain base^(2j/d).
        let json = """
        {
            "vocab_size": 32, "hidden_size": 16, "moe_intermediate_size": 8,
            "num_hidden_layers": 2, "num_attention_heads": 2, "head_dim": 8,
            "q_lora_rank": 8, "qk_rope_head_dim": 4, "rms_norm_eps": 1e-6,
            "o_groups": 1, "o_lora_rank": 8, "sliding_window": 128,
            "compress_ratios": [4, 8], "compress_rope_theta": 160000.0,
            "rope_scaling": {"type": "yarn", "factor": 1,
                "original_max_position_embeddings": 64,
                "beta_fast": 32, "beta_slow": 1},
            "n_routed_experts": 4, "n_shared_experts": 1, "num_experts_per_tok": 2,
            "scoring_func": "sqrtsoftplus", "routed_scaling_factor": 1.5,
            "swiglu_limit": 10.0, "num_hash_layers": 0,
            "num_nextn_predict_layers": 0, "norm_topk_prob": true,
            "hc_mult": 2, "hc_sinkhorn_iters": 3, "hc_eps": 1e-6,
            "rope_theta": 10000.0, "max_position_embeddings": 4096
        }
        """
        return try! JSONDecoder().decode(
            DeepseekV4Configuration.self, from: json.data(using: .utf8)!)
    }

    private func loadGolden() throws -> [String: Any] {
        guard let data = FileManager.default.contents(atPath: Self.goldenPath) else {
            throw XCTSkip("golden vectors not present at \(Self.goldenPath)")
        }
        return try JSONSerialization.jsonObject(with: data) as! [String: Any]
    }

    private func floats2D(_ any: Any) -> MLXArray {
        let rows = any as! [[Double]]
        let flat = rows.flatMap { $0.map { Float($0) } }
        return MLXArray(flat, [rows.count, rows[0].count])
    }

    /// Run one golden case: prefill in `chunks` then decode token-by-token;
    /// compare every emitted slot against the official values.
    private func runCase(_ c: [String: Any], ratio: Int, chunks: [Int]) throws {
        let config = goldenConfig()
        let comp = DeepseekV4Compressor(config: config, ratio: ratio)
        let w = c["weights"] as! [String: Any]
        let ape = floats2D(w["ape"]!)
        let normW = (w["norm.weight"] as! [Double]).map { Float($0) }
        try comp.update(
            parameters: ModuleParameters.unflattened([
                "wkv.weight": floats2D(w["wkv.weight"]!),
                "wgate.weight": floats2D(w["wgate.weight"]!),
                "norm.weight": MLXArray(normW),
                "ape": ape,
            ]), verify: .noUnusedKeys)

        let x2d = floats2D(c["x"]!)                      // [T, dim]
        let T = x2d.dim(0)
        let prefillLen = (c["prefill_len"] as! NSNumber).intValue
        let cache = DSAKVCache(windowSize: 128, ratio: ratio)

        // prefill in the given chunk pattern (sums to prefillLen)
        var pos = 0
        for len in chunks {
            let chunk = x2d[pos ..< (pos + len), 0...].expandedDimensions(axis: 0)
            comp.process(x: chunk, cache: cache)
            pos += len
        }
        XCTAssertEqual(pos, prefillLen)
        // decode the rest token by token
        while pos < T {
            comp.process(x: x2d[pos ..< (pos + 1), 0...].expandedDimensions(axis: 0), cache: cache)
            pos += 1
        }

        let slots = c["slots"] as! [[String: Any]]
        guard let compressed = cache.slotsView else {
            XCTFail("no slots emitted"); return
        }
        XCTAssertEqual(cache.slotCount, slots.count, "slot count mismatch")
        for (k, slot) in slots.enumerated() {
            let expected = (slot["value"] as! [Double]).map { Float($0) }
            let got = compressed[0, 0, k, 0...].asArray(Float.self)
            for (i, (e, g)) in zip(expected, got).enumerated() {
                // Tolerance: Metal fast-math exp in softmax differs from torch's
                // libm exp by ~1e-3 rel (verified: fp64 reimplementation matches
                // the official values to 3e-7; the structure is exact and chunked
                // processing is bit-identical to one-shot).
                XCTAssertEqual(g, e, accuracy: max(6e-3, abs(e) * 6e-3),
                               "case ratio=\(ratio) slot \(k) dim \(i)")
            }
        }
    }

    func testCompressorOverlapRatio4MatchesOfficial() throws {
        let g = try loadGolden()
        let c = g["case_a"] as! [String: Any]
        try runCase(c, ratio: 4, chunks: [11])           // one-shot prefill
        try runCase(c, ratio: 4, chunks: [5, 6])         // chunked prefill (crosses block boundary)
        try runCase(c, ratio: 4, chunks: [3, 4, 4])      // unaligned chunks
    }

    func testCompressorNonOverlapRatio8MatchesOfficial() throws {
        let g = try loadGolden()
        let c = g["case_b"] as! [String: Any]
        try runCase(c, ratio: 8, chunks: [20])
        try runCase(c, ratio: 8, chunks: [7, 13])
    }
}
