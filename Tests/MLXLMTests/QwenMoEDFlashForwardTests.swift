...
@Suite("Qwen3MoE forwardForDFlash")
struct QwenMoEDFlashForwardTests {
    private func tinyQwen3MoEConfig(hiddenLayers: Int = 3) throws -> Qwen3MoEConfiguration {
        let json = """
        {
        ...
        return try JSONDecoder.json5().decode(
            Qwen3MoEConfiguration.self, from: Data(json.utf8))
    }

    @Test func logitsMatchPlainForward() throws {
        try Device.withDefaultDevice(.cpu) {
            let model = Qwen3MoEModel(try tinyQwen3MoEConfig())
            eval(model)
