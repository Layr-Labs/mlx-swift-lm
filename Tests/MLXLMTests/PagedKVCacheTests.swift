...
//
// Ported from ekryski/mlx-swift-lm PR #97, adapted to this repo's
// `KVCacheSimple` reference cache.
...
struct PagedKVCacheTests {
    ...
    func `update output matches KVCacheSimple element-wise`() throws {
        ...
        for (idx, n) in chunkSizes.enumerated() {
            ...
            #expect(
                MLX.allClose(pK, sK, atol: 1e-6).item(Bool.self),
                "step \(idx): K values diverged from KVCacheSimple")
            #expect(
                MLX.allClose(pV, sV, atol: 1e-6).item(Bool.self),
